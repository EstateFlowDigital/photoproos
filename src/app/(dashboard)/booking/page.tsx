...
export default async function BookingPage() {
  ...
  const publishedCount = bookingForms.filter((form) => form.isPublished).length;
  const totalSubmissions = bookingForms.reduce((sum, form) => sum + form.submissionCount, 0);
  const totalBookings = bookingForms.reduce((sum, form) => sum + form.bookingCount, 0);
    ...
    <div className="space-y-6" data-element="booking-dashboard-page">
        ...
        actions={
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              ...
              <span className="hidden md:inline">New Booking Form</span>
            </Link>
          </div>
        }
      />
      ...
      <div className="auto-grid grid-min-200 grid-gap-3">
        <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4">
          <p className="text-xs font-medium uppercase tracking-wider text-foreground-muted">Total Forms</p>
          <p className="mt-2 text-2xl font-bold text-foreground">{bookingForms.length}</p>
        </div>
        <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-4 lg:col-span-2">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-lg font-semibold text-foreground">Recent Booking Forms</h2>
            <Link
              href="/scheduling/booking-forms"
          ...
          {recentForms.length === 0 ? (
            <div className="rounded-xl border border-dashed border-[var(--card-border)] bg-[var(--card)] p-8 text-center">
              ...
              <p className="mt-3 text-sm font-medium text-foreground">No booking forms yet</p>
              <p className="mt-1 text-xs text-foreground-muted">
                Create a booking form to start taking client requests.
              </p>
              <Link
                href="/scheduling/booking-forms"
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-6">
            <h3 className="text-sm font-semibold text-foreground">Booking Workflow</h3>
            <ul className="mt-3 space-y-2 text-sm text-foreground-secondary">
              <li className="flex items-start gap-2">
                <CheckIcon className="mt-0.5 h-4 w-4 text-[var(--success)]" />
                Share a public link for client requests.
              </li>
              <li className="flex items-start gap-2">
                <CheckIcon className="mt-0.5 h-4 w-4 text-[var(--success)]" />
                Set approval rules before bookings convert.
              </li>
              <li className="flex items-start gap-2">
                <CheckIcon className="mt-0.5 h-4 w-4 text-[var(--success)]" />
                Route submissions to your calendar.
              </li>
            </ul>
          </div>
          <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-6">
            ...
            <div className="mt-3 space-y-2">
              ...
              <Link
                ...
                className="flex items-center gap-2 rounded-lg border border-[var(--card-border)] px-3 py-2 text-sm text-foreground transition-colors hover:bg-[var(--background-hover)]"
              >
                <ScheduleIcon className="h-4 w-4 text-foreground-muted" />
                Scheduling Calendar
              </Link>
            </div>
          </div>
...
function ScheduleIcon({ className }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={className}>
      <path fillRule="evenodd" d="M6 2.75a.75.75 0 0 1 .75.75V5h6V3.5a.75.75 0 0 1 1.5 0V5h.25A2.75 2.75 0 0 1 17 7.75v7.5A2.75 2.75 0 0 1 14.25 18h-8.5A2.75 2.75 0 0 1 3 15.25v-7.5A2.75 2.75 0 0 1 5.75 5H6V3.5a.75.75 0 0 1 .75-.75Zm0 6.5a.75.75 0 0 1 .75-.75h6.5a.75.75 0 0 1 0 1.5h-6.5A.75.75 0 0 1 6 9.25Zm0 3a.75.75 0 0 1 .75-.75h3.5a.75.75 0 0 1 0 1.5h-3.5a.75.75 0 0 1-.75-.75Z" clipRule="evenodd" />
    </svg>
  );
}
