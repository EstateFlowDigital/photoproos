...
export function BulkPdfButton({ paidCount }: BulkPdfButtonProps) {
  ...
  const handleExport = async () => {
    ...
    setIsExporting(true);
    try {
      const response = await fetch("/api/payments/bulk-pdf", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "Failed to export receipts");
      }
      ...
      showToast(error instanceof Error ? error.message : "Failed to export receipts", "error");
    } finally {
