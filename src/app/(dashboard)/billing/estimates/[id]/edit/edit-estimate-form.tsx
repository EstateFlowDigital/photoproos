...
import type { LineItemType, Estimate, EstimateLineItem } from "@prisma/client";
...
export function EditEstimateForm({ estimate, clients, services }: EditEstimateFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showToast(result.error || "Failed to update estimate", "error");
      }
    } catch {
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full sm:w-48 rounded-lg border border-[var(--card-border)] bg-[var(--background)] px-4 py-2.5 text-foreground focus:border-[var(--primary)] focus:outline-none focus:ring-1 focus:ring-[var(--primary)]"
            />
          </div>
          <div>
