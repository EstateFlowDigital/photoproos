...
const demoWebsite = {
  ...
  description: "Welcome to this beautiful 4-bedroom home featuring an open floor plan, gourmet kitchen with granite countertops, and a private backyard oasis with pool. Located in one of Austin's most sought-after neighborhoods with excellent schools and easy access to downtown.",
  features: [
    "Open floor plan with high ceilings",
    "Gourmet kitchen with granite countertops",
    "Primary suite with spa-like bathroom",
    "Private backyard with heated pool",
    "Three-car garage",
...
const demoLeads = [
  {
    ...
    phone: "(512) 555-1234",
    message: "I'm very interested in this property. Is it still available? I'd like to schedule a viewing.",
    status: "new",
  ...
  },
  {
    ...
    phone: "(512) 555-5678",
    message: "Beautiful home! Can you send me more information about the HOA fees?",
    status: "contacted",
  ...
  },
  {
    ...
    phone: null,
    message: "What are the property taxes?",
    status: "qualified",
...
const demoAnalytics = {
  ...
  dailyData: Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      date,
      pageViews: Math.floor(Math.random() * 60) + 20,
      uniqueVisitors: Math.floor(Math.random() * 50) + 15,
    };
  }),
};
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            ...
            <div className="space-y-6 lg:col-span-2">
              ...
              <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-6">
                ...
                <p className="text-foreground-secondary">{website.description}</p>
                    ...
                    {website.virtualTourUrl && (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-sm text-foreground-muted">Matterport</p>
                        </div>
                      </a>
                    )}
                    {website.videoUrl && (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-sm text-foreground-muted">YouTube</p>
                        </div>
                      </a>
                    )}
                  </div>
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-[var(--primary)]">{website._count.leads}</p>
                    <p className="text-sm text-foreground-muted">Leads</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-foreground">{demoAnalytics.totals.tourClicks}</p>
                    <p className="text-sm text-foreground-muted">Tour Clicks</p>
                  </div>
                  <div>
                    <p className="text-2xl font-bold text-foreground">{demoAnalytics.totals.socialShares}</p>
        ...
        {activeTab === "leads" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-foreground">Leads ({demoLeads.length})</h2>
              <div className="flex items-center gap-2">
                <select className="h-9 rounded-lg border border-[var(--card-border)] bg-[var(--card)] px-3 text-sm text-foreground">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-5">
              <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4">
                ...
                <p className="mt-1 text-2xl font-bold text-foreground">
                  {demoAnalytics.totals.pageViews}
                </p>
              </div>
              <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4">
                <p className="text-sm text-foreground-muted">Unique Visitors</p>
                <p className="mt-1 text-2xl font-bold text-foreground">
                  {demoAnalytics.totals.uniqueVisitors}
                </p>
              </div>
              <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4">
                <p className="text-sm text-foreground-muted">Tour Clicks</p>
                <p className="mt-1 text-2xl font-bold text-foreground">
                  {demoAnalytics.totals.tourClicks}
                </p>
              </div>
              <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4">
                <p className="text-sm text-foreground-muted">Photo Views</p>
                <p className="mt-1 text-2xl font-bold text-foreground">
                  {demoAnalytics.totals.photoViews.toLocaleString()}
                </p>
              </div>
              <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-4">
                <p className="text-sm text-foreground-muted">Social Shares</p>
                <p className="mt-1 text-2xl font-bold text-foreground">
                  {demoAnalytics.totals.socialShares}
                </p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-6">
              ...
              <div className="flex h-48 items-end gap-2">
                {demoAnalytics.dailyData.map((day, i) => (
                  <div key={i} className="flex-1">
                    <div
                      className="mx-auto w-full max-w-[40px] rounded-t bg-[var(--primary)]"
                      style={{ height: `${(day.pageViews / 80) * 100}%` }}
                    />
            ...
            <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-6">
              ...
              <div className="space-y-4">
                <label className="flex items-center justify-between">
                  ...
                  <button
                    className={`relative h-6 w-11 rounded-full transition-colors ${
                      website.isBranded ? "bg-[var(--primary)]" : "bg-[var(--background-tertiary)]"
                    }`}
                  >
                    <span
                      className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${
                        website.isBranded ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </label>
                ...
                <label className="flex items-center justify-between">
                  ...
                  <button
                    className={`relative h-6 w-11 rounded-full transition-colors ${
                      website.showPrice ? "bg-[var(--primary)]" : "bg-[var(--background-tertiary)]"
                    }`}
                  >
                    <span
                      className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${
                        website.showPrice ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </label>
                ...
                <label className="flex items-center justify-between">
                  ...
                  <button
                    className={`relative h-6 w-11 rounded-full transition-colors ${
                      website.showAgent ? "bg-[var(--primary)]" : "bg-[var(--background-tertiary)]"
                    }`}
                  >
                    <span
                      className={`absolute left-0.5 top-0.5 h-5 w-5 rounded-full bg-white transition-transform ${
                        website.showAgent ? "translate-x-5" : "translate-x-0"
                      }`}
                    />
                  </button>
                </label>
